...
function colorRiskSettings() {
    const risk_items = document.querySelectorAll(".settings-row > label");
    Array.from(risk_items).forEach((el) => {
        if (el.textContent.toLowerCase().includes("risks")) {
            el.closest(".settings-row").dataset.risk = "true";
        }
    });
}
...
function checkAdminToken() {
    ...
    if (!disable_admin_token.checked && !admin_token.value.startsWith("$argon2")) {
        ...
        // When closing the banner, store the current date/time in the browser
        const admin_token_warning = document.getElementById("admin_token_warning");
        const closeBtn = document.getElementById("admin_token_warning_close");
        if (closeBtn) {
            closeBtn.addEventListener("click", function() {
                admin_token_warning.classList.add("d-none");
                const d = new Date();
