...
use crate::{
    CONFIG,
    api::{self, EmptyResult, JsonResult, Notify, PasswordOrOtpData, UpdateType},
    auth::ClientVersion,
    auth::Headers,
    config::PathType,
    crypto,
...
async fn sync(data: SyncData, headers: Headers, client_version: Option<ClientVersion>, conn: DbConn) -> JsonResult {
    ...
    let cipher_sync_data = CipherSyncData::new(&headers.user.uuid, &conn).await;
...
async fn post_ciphers(data: Json<CipherData>, headers: Headers, conn: DbConn, nt: Notify<'_>) -> JsonResult {
    ...
    update_cipher_from_data(&mut cipher, data, &headers, &conn, &nt, UpdateType::SyncCipherCreate).await?;

    Ok(Json(cipher.to_json(&headers.host, &headers.user.uuid, None, &conn).await?))
}

/// umewarden has no organizations, so there is no "personal ownership" org
/// policy to enforce - this is a no-op kept only so call sites elsewhere
/// don't need to be touched.
async fn enforce_personal_ownership_policy(_data: Option<&CipherData>, _headers: &Headers, _conn: &DbConn) -> EmptyResult {
    Ok(())
}
...
pub async fn update_cipher_from_data(
    ...
    if data.organization_id.is_some() {
        err!("This umewarden build has no organizations - items cannot be moved into one")
    }
...
async fn post_ciphers_import(data: Json<ImportData>, headers: Headers, conn: DbConn, nt: Notify<'_>) -> EmptyResult {
    ...
    for (index, mut cipher_data) in data.ciphers.into_iter().enumerate() {
        ...
        update_cipher_from_data(&mut cipher, cipher_data, &headers, &conn, &nt, UpdateType::None).await?;
    }
...
async fn put_cipher(
    ...
    update_cipher_from_data(&mut cipher, data, &headers, &conn, &nt, UpdateType::SyncCipherUpdate).await?;
...
async fn share_cipher_by_uuid(
    ...
    if data.cipher.organization_id.is_some() || !data.collection_ids.is_empty() {
        err!("This umewarden build has no organizations - items cannot be shared into one")
    }
    ...
    update_cipher_from_data(&mut cipher, data.cipher, headers, conn, nt, ut).await?;
...
async fn save_attachment(
    ...
    let size_limit = if let Some(ref user_id) = cipher.user_uuid {
        ...
        err!("Cipher is not owned by a user");
    };
...
async fn delete_cipher_attachment_by_id(
    ...
    let cipher_json = cipher.to_json(&headers.host, &headers.user.uuid, None, conn).await?;
...
impl CipherSyncData {
    pub async fn new(user_id: &UserId, conn: &DbConn) -> Self {
        ...
        let cipher_archives: HashMap<CipherId, NaiveDateTime> =
            Archive::find_by_user(user_id, conn).await.into_iter().collect();

        // Generate a list of Cipher UUID's containing a Vec with one or more Attachment records
        let attachments = Attachment::find_all_by_user(user_id, conn).await;
