...
pub struct SetPasswordData {
    ...
    master_password_hint: Option<String>,
    #[allow(dead_code)]
    org_identifier: Option<String>, // Unused; accepted so older clients' payloads still parse.
}
...
async fn is_email_2fa_required(_member_id: Option<()>, _conn: &DbConn) -> bool {
    ...
    CONFIG.email_2fa_enforce_on_verified_invite()
}
...
pub async fn register(data: Json<RegisterData>, email_verification: bool, conn: DbConn) -> JsonResult {
    ...
    if CONFIG.mail_enabled() {
        ...
        if email_verified && is_email_2fa_required(None, &conn).await {
