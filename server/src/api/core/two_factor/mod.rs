...
use crate::{
    ...
    db::{
        DbConn, DbPool,
        models::{DeviceType, TwoFactor, TwoFactorIncomplete, TwoFactorType, User, UserId},
    },
    mail,
    util::NumberOrString,
};
...
/// umewarden has no organizations, so there is no "require 2FA" org policy to
/// enforce - this is a no-op kept only so call sites elsewhere don't need to
/// be touched.
