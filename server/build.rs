...
fn main() {
    // These allow using e.g. #[cfg(sqlite)] instead of #[cfg(feature = "sqlite")], which helps when trying to add them through macros
    #[cfg(feature = "sqlite_system")] // The `sqlite` feature implies this one.
    println!("cargo:rustc-cfg=sqlite");
    #[cfg(not(feature = "sqlite_system"))]
    compile_error!("umewarden only supports the sqlite DB backend. Build with: cargo build --release");
