...
use crate::{
    CONFIG,
    api::{EmptyResult, JsonResult, PasswordOrOtpData, core::two_factor::generate_recover_code},
    auth::{ClientHeaders, Headers},
    crypto,
    db::{
        DbConn,
        models::{AuthRequest, AuthRequestId, DeviceId, TwoFactor, TwoFactorType, User, UserId},
    },
    error::{Error, EventType, MapResult},
    mail,
};
...
async fn send_email_login(data: Json<SendEmailLoginData>, client_headers: ClientHeaders, conn: DbConn) -> EmptyResult {
    ...
    let user = if let Some(email) = email {
        ...
        // No password hash was submitted, so fall back to the most recently used device.
