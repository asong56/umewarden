...
use crate::{
    ...
    db::{
        DbConn,
        models::DeviceId,
        schema::{invitations, twofactor_incomplete, users},
    },
    error::MapResult,
...
use super::{
    Cipher, Device, Favorite, Folder, TwoFactor, TwoFactorIncomplete,
};
...
impl User {
    pub async fn to_json(&self, conn: &DbConn) -> Value {
        // umewarden has no organizations - always empty.
        let orgs_json: Vec<Value> = Vec::new();
    ...
    pub async fn get_all(conn: &DbConn) -> Vec<Self> {
        conn.run(move |conn| users::table.select(Self::as_select()).load(conn).expect("Error loading users")).await
    }
